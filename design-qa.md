...
## Edge context-menu regression — 2026-09-10

- The menu previously anchored to the narrow animated hosting view, while delayed detail presentation and direct hover callbacks could continue during menu tracking. Switched to a detached screen-space menu with a 200 pt minimum width and an anchor clamped inside the active display; opening it cancels pending hover work and hides the detail card. AppKit receives menu clicks and Escape without rail interception.
- Positioning follows Apple's [NSMenu screen-coordinate API](https://developer.apple.com/documentation/appkit/nsmenu/popup(positioning:at:in:)). Existing menu actions and panel levels are retained; dismissal restores normal rail layout and hover handling.
- `swift test -c release`: 210 tests across 20 suites passed, including full menu bounds at four edges and a display with negative origin. Universal `0.3.2-dev` packaging completed; the installed ZIP extraction passes strict/deep codesign verification. `pkgbuild` printed `write: Permission denied` warnings before successfully writing its package; the locally installed artifact is the independently verified ZIP, not the PKG.
- After restarting only the canonical application, CUA confirmed DeepSeek is selected in General while Codex remains preferred. The automation captured the rail but could not reliably observe the context-menu popup, and further UI actions were interrupted by user activity. Actual right-click popup appearance remains for user confirmation; geometry tests are not a full visual acceptance.
- Before the final menu build, one running process sampled 0.3% CPU and about 246 MiB RSS. This is a point-in-time reading, not a sustained resource guarantee.
