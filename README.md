...
待机只留当前首选环：外环是周额度，有 5 小时窗口的服务商再加一圈内环。悬停展开已选服务商，并向内打开详情：分组列表里的大号百分比、重置倒计时、套餐标签；Claude 还会显示云端会话额度（Cloud credits）。正在运行的任务按项目分组列在最下面，样式参照 Claude 侧栏。菜单栏显示单色图标加今天的估算金额，左键打开消耗概览，右键打开设置。
...
| 配额环 | 外环周额度、内环 5 小时额度；“按用量”或“按圆环”配色，可自定义颜色，默认跟随系统强调色 |
| 详情 | 分组列表：大号百分比、进度条、重置倒计时、套餐；Claude 显示 Cloud credits 剩余金额与到期日 |
| 运行任务 | 按项目分组、每个任务一行，参照 Claude 侧栏；Claude 任务显示桌面 App 的会话标题，无运行任务时不显示 |
| 本机读取 | Codex、Claude、Cursor、Gemini、Antigravity、Copilot、Kimi Code、Grok 读本机登录；ClinePass、Z.ai 也可在设置里填密钥 |
| 设置 | 系统侧栏样式：通用 / 消耗 / 关于 / 服务商；圆环样式实时预览 |
| 自动更新 | 关于页发现新版本可“下载并安装”：SHA256SUMS 校验、版本与签名检查、失败回滚，装好自动重开 |
| 消耗概览 | 菜单栏显示今日估算金额；点击后在下方打开紧凑账单，按今天 / 近 7 天 / 本月查看汇总 |
| 图表与明细 | 用量构成、独立近 30 天趋势、近 81 天活动热力图（附活跃天数、最长连续、单日峰值）；悬停查看当天模型用量与估算金额 |
...
| 菜单栏入口 | 单色图标 + 金额（余额用 ¥ / $，今日估算带火焰图标）；左键消耗概览、右键设置；隐藏侧栏后可从这里恢复 |
| 低开销 | 本机日志增量解析，只读新增内容；分钟扫描约 1.65 s → 0.07 s，CPU 约 18% → 2% |
...
6. 点槽外的设置齿轮，或右键菜单栏入口打开设置。左侧是通用 / 消耗 / 关于 / 服务商列表；在“关于”里发现新版本时可以一键下载并安装。左键菜单栏入口在图标下方打开本机 token 账单。
7. 悬停某家环时，若该服务商有正在运行的任务，详情底部按项目分组列出：灰色项目名，下面每个任务一行（绿色运行圈 + 标题），悬停看全文。
...
详情卡底部列出正在运行的任务，样式参照 Claude 侧栏：按项目分组，灰色小字是项目名，每个任务一行，前面是绿色运行圈，标题过长时省略，悬停高亮并显示全文。Claude 任务读取桌面 App（Code 标签页）的会话标题和起始项目，worktree 会归到原仓库下。没有运行中的任务时整块不显示；卡片按内容伸缩，超过屏幕可用高度后可滚动。

## 自动更新（0.3.7）

设置 → 关于发现新版本时点“下载并安装”：下载 Release 中的 zip 并显示进度，按同一 Release 的 `SHA256SUMS` 校验，解压后确认版本号与代码签名，再替换当前应用并重新打开。校验失败不会改动已安装版本，替换失败会恢复旧版。应用所在文件夹没有写入权限、不是从 .app 运行或被 macOS 放在隔离位置时，仍提供“打开发布页”。

- 0.3.6 及更早版本需手动安装一次 0.3.7 以上版本。
- 应用是 ad-hoc 签名，每次构建签名不同，更新后首次读取钥匙串时 macOS 可能再次请求授权。
- SHA256 校验防下载损坏，不防整个 Release 被替换。
...
| 0.3.7 | 关于页一键下载安装更新；运行任务改为 Claude 侧栏样式；Claude 凭据过期给出刷新提示 |
| 0.3.6 | 外壳恢复 0.3.3 形状；新应用图标 |
| 0.3.5 | 外壳圆角回到 22pt；悬停设置按钮挂在外壳末端之外 |
| 0.3.4 | 5 小时内环与自定义圆环配色；详情卡、消耗面板、设置窗口重做；日志增量解析；Opus 5.5 / GPT-6 Sol 单价；菜单栏单色图标；活动图统计 |
...
- 小工具可在 **通用 → 侧边栏服务商** 中勾选 **DeepSeek** 或 **自定义中转站**，也可设为首选。图标下显示原币种余额，悬停查看完整金额、充值 / 赠送余额及更新时间；余额没有额度上限，因此不显示百分比。同类账户按币种汇总，多币种时展开查看。小工具与菜单栏共用余额缓存，不增加自动查询频率。
...
| Claude | 有 `~/.claude/.credentials.json` 会自动拉；只有钥匙串时，第一次点详情里的 Connect。凭据由 Claude CLI 负责刷新，过期时用 `claude` 发一条消息即可恢复 |
