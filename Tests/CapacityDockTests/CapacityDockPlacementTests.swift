...
struct CapacityDockPlacementTests {
    ...
    @Test("orb stack hit tests each ring column and ignores the empty card")
    func orbStackHitGeometry() {
        let bounds = CGRect(x: 0, y: 0, width: 52, height: 200)
        ...
        #expect(frames[0].origin == CGPoint(x: 6, y: 6))
        #expect(frames[0].size == CGSize(width: 40, height: 58))
        #expect(CapacityDockOrbStack.contains(CGPoint(x: 26, y: 30), frames: frames))
        #expect(!CapacityDockOrbStack.contains(CGPoint(x: 2, y: 30), frames: frames))
        #expect(!CapacityDockOrbStack.contains(CGPoint(x: 26, y: 70), frames: frames))
    }
