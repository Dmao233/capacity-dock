...
struct CapacityDockPreferencesTests {
    ...
    @Test("fresh installs enable the dock with Codex as the only resting provider")
    func freshDefaults() {
        ...
        #expect(snapshot.scale == 1.0)
        #expect(snapshot.theme == .graphite)
        #expect(snapshot.gaugeShape == .circle)
        #expect(snapshot.keepExpanded == false)
    }

    @Test("keep-expanded rest persists independently from placement")
    func persistsKeepExpanded() {
        let defaults = defaults()

        CapacityDockPreferences.setKeepExpanded(true, defaults: defaults)
        #expect(CapacityDockPreferences.load(defaults: defaults).keepExpanded == true)

        CapacityDockPreferences.setKeepExpanded(false, defaults: defaults)
        #expect(CapacityDockPreferences.load(defaults: defaults).keepExpanded == false)
    }
    ...
    func supportsCompleteProviderCatalog() {
        #expect(CapacityDockPreferences.supportedProviders.count == ProviderConnectionCatalog.providers.count)
    ...
    @Test("legacy card-rail default scale migrates to Codenotch native size")
    func migratesLegacyDefaultScale() {
        let defaults = defaults()
        defaults.set(0.6, forKey: CapacityDockPreferences.scaleKey)
