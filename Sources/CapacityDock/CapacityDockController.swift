...
final class CapacityDockController {
    private let store: CapacityDockStore
    ...
    private var pointerInsideCap = false
    private var hoveredRailItem: CapacityDockRailItem?
    ...
    func start() {
        ...
        spaceObserver = NSWorkspace.shared.notificationCenter.addObserver(
            ...
            Task { @MainActor [weak self] in self?.activeSpaceDidChange() }
        }
    ...
    private func ensureRailPanel() {
        ...
        hosting.interactiveShapeContains = { [weak model] point, bounds in
            ...
            return model.containsPointer(swiftUIPoint, in: bounds)
        }
    ...
    private func startEventMonitoring() {
        ...
        let timer = Timer(timeInterval: 1.0 / 60.0, repeats: true) { [weak self] _ in
            MainActor.assumeIsolated {
                ...
                let pointerMoved = point != self.lastPolledPointer
                self.lastPolledPointer = point
                if !pointerMoved {
                    // A missed enter (Spaces, ignoresMouseEvents flipping)
                    // must recover even if the cursor is parked on the rail.
                    let overRail = self.railPanel?.isVisible == true
                        && self.railPanel?.frame.contains(point) == true
        ...
        localEventMonitor = NSEvent.addLocalMonitorForEvents(
            ...
            if event.type == .mouseMoved {
                ...
                let point = NSEvent.mouseLocation
                if self.railContains(screenPoint: point) {
                    self.presentRailContextMenu(at: point)
                    return nil
                }
    ...
    private func syncPointerHover(at point: CGPoint = NSEvent.mouseLocation) {
        ...
        let local = railPanel.map { swiftUILocalPoint(point, in: $0.frame) } ?? point
        let insideCap = model.containsSettingsCap(local, in: bounds)
        let insideRail = railContains(screenPoint: point) && !insideCap
        let insideDetail = detailPanel?.isVisible == true && detailContains(screenPoint: point)
        let row = insideRail ? railItem(at: point) : nil

        if insideCap != pointerInsideCap {
            pointerInsideCap = insideCap
            settingsCapHoverChanged(insideCap)
        }
        ...
        if row != hoveredRailItem {
            let previous = hoveredRailItem
            hoveredRailItem = row
            if case .provider(let provider) = previous {
                providerHoverChanged(provider, hovering: false)
            }
            if case .provider(let provider) = row {
                providerHoverChanged(provider, hovering: true)
            }
        } else if case .provider(let provider) = row {
            let showing = detailPanel?.isVisible == true
                && model.hoveredProvider == provider
                && !detailIsDismissing
            if !showing, detailWork == nil {
                providerHoverChanged(provider, hovering: true)
            }
        }
    }
    ...
    private func activeSpaceDidChange() {
        ...
        recoverPointerTracking()
        if let provider = model.hoveredProvider, detailPanel?.isVisible == true {
            layoutDetail(for: provider, transaction: .immediate)
        }
    }

    private func recoverPointerTracking() {
        let point = NSEvent.mouseLocation
    ...
    private func providerHoverChanged(_ provider: CapacityDockProvider, hovering: Bool) {
        ...
        if hovering {
            ...
            collapseWork?.cancel()
            let alreadyShowing = detailPanel?.isVisible == true
            ...
            // Stay on the last card while crossing the gap between rings.
            if pointerInsideRail { return }
            let work = DispatchWorkItem { [weak self] in
                guard let self, !self.model.interaction.isDetailHovered else { return }
                guard !self.pointerInsideRail, !self.pointerInsideCap else { return }
    ...
    private func settingsCapHoverChanged(_ hovering: Bool) {
        ...
        model.settingsCapProgress = hovering ? 1 : 0
        if hovering {
            expansionWork?.cancel()
            hideDetail(animated: true)
        } else if !pointerInsideRail, !pointerInsideDetail {
            scheduleCollapse()
        }
    }
    ...
    private func presentRailContextMenu(at screenPoint: CGPoint) {
        ...
        func addItem(
            ...
            let proxy = CapacityDockRailMenuProxy(handler)
            proxies.append(proxy)
            let item = NSMenuItem(
                title: NSLocalizedString(title, comment: ""),
                action: #selector(CapacityDockRailMenuProxy.invoke(_:)),
                keyEquivalent: ""
            )
            item.target = proxy
            item.state = state
        ...
        menu.addItem(addItem(
            ...
            self?.toggleKeepExpanded()
        })
        let dockMenu = NSMenu()
        dockMenu.addItem(addItem("Left") { [weak self] in self?.dock(to: .left) })
        dockMenu.addItem(addItem("Right") { [weak self] in self?.dock(to: .right) })
        dockMenu.addItem(addItem("Top") { [weak self] in self?.dock(to: .top) })
        dockMenu.addItem(addItem("Bottom") { [weak self] in self?.dock(to: .bottom) })
        let dockItem = NSMenuItem(
            title: NSLocalizedString("Dock to Edge", comment: ""),
            action: nil,
            keyEquivalent: ""
        )
    ...
    private func toggleKeepExpanded() {
        CapacityDockPreferences.setKeepExpanded(
            !model.preferences.keepExpanded,
            defaults: defaults
        )
        collapseWork?.cancel()
        if !model.preferences.keepExpanded,
           !pointerInsideRail,
           !pointerInsideDetail,
           !pointerInsideCap {
            scheduleCollapse()
    ...
    private func layoutDetail(
        ...
        let row = frames[index]
        let providerAxisMidpoint: CGFloat = if model.isVertical {
            railPanel.frame.maxY - row.midY
        } else {
            railPanel.frame.minX + row.midX
        }
...
private final class CapacityDockRailMenuProxy: NSObject {
...
private final class CapacityDockPanel: NSPanel {
    init() {
        ...
        // `stationary` keeps the window tied to the space where it was first
        // shown, so hover/detail events go missing on other desktops until a
        // space switch reattaches it. `canJoinAllSpaces` already keeps the
        // rail at the same screen position on every desktop.
