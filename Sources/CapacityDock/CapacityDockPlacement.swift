...
/// with no enclosing card.
enum CapacityDockOrbStack {
    static func itemFrames(
        ...
        return (0..<itemCount).map { index in
            ...
            return CGRect(x: along, y: crossPad, width: rowHeight, height: crossSpan)
        }
    }

    static func contains(_ point: CGPoint, frames: [CGRect]) -> Bool {
