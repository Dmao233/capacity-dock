...
enum CapacityDockPreferences {
    static let enabledKey = "CapacityDockEnabled"
    static let selectedProvidersKey = "CapacityDockProviders"
    static let preferredProviderKey = "CapacityDockPreferredProvider"
    static let dockEdgeKey = "CapacityDockEdge"
    static let attachmentEdgeKey = "CapacityDockAttachmentEdge"
    static let legacyDockedKey = "CapacityDockDocked"
    static let normalizedHorizontalOffsetKey = "CapacityDockHorizontalOffset"
    static let normalizedVerticalOffsetKey = "CapacityDockVerticalOffset"
    static let scaleKey = "CapacityDockScale"
    ...
    static func load(defaults: UserDefaults = .standard) -> Snapshot {
        migrateLegacyShellIfNeeded(defaults: defaults)
    ...
    static func setScale(_ scale: Double, defaults: UserDefaults = .standard) {
        ...
        defaults.set(currentShellVersion, forKey: shellVersionKey)
        notifyChanged()
    }
    ...
    static func setKeepExpanded(_ keepExpanded: Bool, defaults: UserDefaults = .standard) {
        defaults.set(keepExpanded, forKey: keepExpandedKey)
        notifyChanged()
    }
    ...
    /// The card-rail shell used 0.6 as the implicit size. The Codenotch orb
    /// stack is native at 1.0, so a leftover default 0.6 is rewritten once.
    private static func migrateLegacyShellIfNeeded(defaults: UserDefaults) {
        guard defaults.integer(forKey: shellVersionKey) < currentShellVersion else { return }
        if defaults.object(forKey: scaleKey) == nil
            || abs(defaults.double(forKey: scaleKey) - 0.6) < 0.0001 {
